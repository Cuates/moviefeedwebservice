...
# Import modules
import moviefeedwebserviceconfig # movie feed web service configuration
...
# Class
class MovieFeedWebServiceClass():
  # Constructor
  def __init__(self):
    pass

  # Extract movie feed
  def _extractMovieFeed(self, type, actionWord, selectColumn, procedure, optionMode, possibleParams, feedStorage):
    ...
    # Catch exceptions
      ...
      # Set message
      returnMessage = [{'SError': 'Error', 'SMessage': 'Caught ' + actionWord + ' movie feed execution failure'}]

      # Log message
      self._setLogger('Issue ' + actionWord + ' movie feed : ' + str(e))

    # Return message
    return returnMessage

  # Insert, Update, or Delete movie feed
  def _insertupdatedeleteMovieFeed(self, type, actionWord, procedure, optionMode, possibleParams, feedStorage, removeParams):
  ...
  def __openConnection(self, type = 'notype'):
    ...
    try:
      ...
      mfwsconfig = moviefeedwebserviceconfig.MovieFeedWebServiceConfig()

      # Set variables based on type
      mfwsconfig._setConfigVars(type)

      # Create empty dictionary
      conVars = {}

      # Get dictionary of values
      conVars = mfwsconfig._getConfigVars()
  ...
  def _setLogDictConfig(self):
    ...
    # Set variables based on type
    mfwsconfig._setFilenameVars('Log')

    # Get dictionary of values
    dictFeedType = mfwsconfig._getFilenameVars()
